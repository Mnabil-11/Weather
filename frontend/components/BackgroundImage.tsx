...
import { backgroundGradients, overlayOpacity, type WeatherTheme } from '@/shared/weatherTheme';
...
export function BackgroundImage({ theme }: { theme: WeatherTheme }) {
  ...
  return (
    <div className={`absolute inset-0 -z-10 bg-gradient-to-br ${backgroundGradients[theme]} transition-colors duration-1000 ease-in-out`}>
      <AnimatePresence>
        {url && (
      ...
      <div className={`absolute inset-0 transition-colors duration-1000 ease-in-out ${overlayOpacity[theme]}`} />
    </div>
  );
}
